	...
	<!--
		CDA validation converter takes True CDA (Instance conformant with generic CDA schema)
		and expands to Templated	format by using typeIds for the element names. Result can then be
		validated with HSCIC DMS specification schemas (must be done as a separate step). A lot of
		the work is to re-order the XML from CDA schema order to the order of
		the current schema generator in use by HSCIC. Usage : transform CDA xml
		...
		Updated Prashant Trivedi v2.1 16/04/2013
	-->
...
<!--	<xsl:template match="@*[name(.)='mediaType']" />
	...
	<xsl:template  match="h:ClinicalDocument"  mode="phase1">
		<xsl:comment>
			...
			ensure that the examples are consistent with the message
			definition contained within the domain message specifications,
			where there are conflicts	with the written message specification or 
			schema, the specification or schema shall be considered to take
			precedence.
		</xsl:comment>	
...
<!--			<xsl:value-of select="npfitlc:messageType/@extension" />-->
	...
	<xsl:template match="h:*[child::h:templateId]"><!-- a class entry point -->
		...
		<!--
			In HSCIC DMS validation schemas elements at entry points to templates have
...
<!--	</xsl:template>-->
	...
	<xsl:template
		match="@*[name(.)='xsi:type' and 
		not(
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'Finding') or
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'Assessment') or 
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'Diagnosis') or 
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'LifeStyle') or 
		contains(parent::node()/preceding-sibling::h:templateId/@extension,'SocialOrPersonalCircumstance')
		)]" />
		...
		<xsl:element name="{name(.)}" namespace="urn:hl7-org:v3"
			...
			<xsl:for-each select="@*">
				<xsl:choose>
					...
					<xsl:when test="name(.)='xsi:type' and not(
						contains(parent::node()/preceding-sibling::h:templateId/@extension,'Finding') or
						contains(parent::node()/preceding-sibling::h:templateId/@extension,'Assessment') or  
						contains(parent::node()/preceding-sibling::h:templateId/@extension,'Diagnosis') or 
						contains(parent::node()/preceding-sibling::h:templateId/@extension,'LifeStyle') or 
	...
	<!--
		...
		peculiarity of HSCIC validation schemas
	-->
	<xsl:template
...
<!--		match="h:participant[@typeCode='SPRF']/@contextControlCode"/> -->
	...
	<!--PRTR1 Code commented to get correct xml element based in messageType
	Should be removed when model are fixed.-->
	<!--<xsl:template match="h:structuredBody/h:component[child::npfitlc:contentId]">
		<component1 xmlns="urn:hl7-org:v3">
			<xsl:call-template name="copyChildren" />
		</component1>
	</xsl:template>-->
	<!--SIKN1 (2015-03-10) added Transfer Of Care message  POCD_MT000026GB01 to test -->
		<xsl:template match="h:structuredBody/h:component[child::npfitlc:contentId]">
		<xsl:choose>
			<xsl:when test="contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT010007GB02')
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT010008GB01')
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT010009GB01')
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT000012GB01')									
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT010010GB01')
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT000020GB01')
									or
									contains(/h:ClinicalDocument/npfitlc:messageType/@extension,'POCD_MT000026GB01')">
